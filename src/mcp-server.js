...
const createMcpServer = (service) => {
  ...
  server.registerTool("create_document", {
    ...
    inputSchema: {
      title: z.string().optional().describe("文档标题（元数据，不进正文）"),
      preset: z.string().optional().describe("样式包名，见 list_presets；缺省用 style profile 或 monthly-report"),
      def: defSchema,
    },
  ...
  server.registerTool("create_document_from_markdown", {
    ...
    inputSchema: {
      ...
      baseDir: z.string().optional().describe("图片相对路径的基准目录，缺省服务进程 cwd"),
      imagesDir: z.string().optional().describe("图片子目录（相对 baseDir），缺省 baseDir 本身"),
  ...
  server.registerTool("render_document", {
    ...
    inputSchema: {
      docId: z.string(),
      outPath: z.string().optional().describe("输出路径，缺省 data/output/<docId>.docx"),
  ...
  server.registerTool("get_outline", {
    ...
    inputSchema: {
      docId: z.string(),
      section: z.string().optional().describe("章节号过滤，如 \"1.1\""),
    },
  ...
  server.registerTool("get_nodes", {
    ...
    inputSchema: { docId: z.string(), ids: z.array(z.string()) },
  ...
  server.registerTool("update_node", {
    ...
    inputSchema: {
      docId: z.string(),
      id: z.string().describe("要替换的节点 id"),
      node: z.record(z.string(), z.any()).describe("新节点（不必带 id，带了也以本参数 id 为准）"),
    },
  ...
  server.registerTool("insert_nodes", {
    ...
    inputSchema: {
      docId: z.string(),
      anchorId: z.string().describe("锚点节点 id（先用 get_outline 定位）"),
      position: z.enum(["before", "after"]).describe("插到锚点之前还是之后"),
      nodes: z.array(z.record(z.string(), z.any())).describe("要插入的节点数组，规范同 create_document 的 contexts"),
    },
  ...
  server.registerTool("delete_nodes", {
    ...
    inputSchema: {
      docId: z.string(),
      ids: z.array(z.string()).describe("要删除的节点 id 数组"),
    },
  ...
  server.registerTool("move_nodes", {
    ...
    inputSchema: {
      docId: z.string(),
      ids: z.array(z.string()).describe("要移动的节点 id 数组，落位顺序以此为准"),
      anchorId: z.string().describe("锚点节点 id"),
  ...
  server.registerTool("get_examples", {
    ...
    inputSchema: {
      topic: z.string().optional().describe("主题名（如 table-span / link / footnote / markdown），缺省列出全部主题"),
    },
  ...
  server.registerTool("register_template", {
    ...
    inputSchema: {
      name: z.string().optional().describe("模板名（便于 list_templates 辨认），缺省用文件名"),
      path: z.string().optional().describe("服务器本地模板路径（与 base64 二选一）"),
      base64: z.string().optional().describe("模板文件 base64（≤5MB，远程上传用；本机文件优先走 path 省上下文）"),
    },
  ...
  server.registerTool("delete_template", {
    ...
    inputSchema: {
      templateId: z.string().describe("要删除的模板 id（见 list_templates）"),
    },
  ...
  server.registerTool("create_document_from_template", {
    ...
    inputSchema: {
      templateId: z.string().describe("register_template 返回的模板 id"),
      title: z.string().optional().describe("文档标题（元数据）"),
      slots: z.record(z.string(), z.any()).optional().describe("{ 槽名: 字符串 | 节点数组 }，槽位清单见 register_template/list_templates 返回"),
      imagesDir: z.string().optional().describe("槽位里图片相对路径的基准目录，缺省服务进程 cwd"),
    },
