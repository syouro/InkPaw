...
| h1PageBreak | bool | false* | 每个一级标题自动另起一页（transform 层补 `pageBreakBefore`）。文档开头、紧跟 newPage / 换页型 sectionBreak 的章不重复补，不会多出空白页；节点显式写了 `pageBreakBefore` 以节点为准。*monthly-report preset 默认 true（报告型文档章起新页），doc meta 写 false 可关 |
